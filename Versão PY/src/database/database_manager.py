...
class DatabaseManager:
    ...
    def initialize_database(self):
        ...
        try:
            ...
            # Marcar horário de inicialização para cálculo de uptime
            import time
    ...
    def get_server_status(self) -> Dict[str, Any]:
        """
        Retorna status do servidor de banco (SQLite) e metadados.
        """
        import time
        import os
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                # Versão do SQLite
                status['sqlite_version'] = conn.execute(text("SELECT sqlite_version()"))\
                    .scalar()

                # Contagens de objetos
                status['tables_count'] = conn.execute(text(
                    "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'"
                )).scalar()
                status['indexes_count'] = conn.execute(text(
            ...
            if os.path.exists(self.db_path):
                ...
                status['file_size_bytes'] = size
                # Tamanho legível
                for unit in ['B','KB','MB','GB','TB']:
                    if size < 1024.0:
                        status['file_size_human'] = f"{size:.2f} {unit}"
                        break
                    size /= 1024.0
                mtime = os.path.getmtime(self.db_path)
                status['last_modified'] = datetime.fromtimestamp(mtime).isoformat(timespec='seconds')

            # Uptime
            if self._start_time:
                up = int(time.time() - self._start_time)
                status['uptime_seconds'] = up
                # Formatar humanizado
                h = up // 3600
                m = (up % 3600) // 60
                s = up % 60
                status['uptime_human'] = f"{h}h {m}m {s}s"
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        """
        Retorna métricas de desempenho e pragmas atuais do SQLite.
        """
        ...
        try:
            ...
            with self.engine.connect() as conn:
                ...
                metrics.update({
                    ...
                    'database_size_bytes': int((page_count or 0) * (page_size or 0)),
                })
            ...
            self.logger.error(f"Erro ao obter métricas de desempenho: {e}")
            return metrics

    def get_top_tables_by_rows(self, limit: int = 5) -> List[Dict[str, Any]]:
        """
        Lista as tabelas com maior número de linhas (aproximação de tamanho).
        """
            ...
            with self.engine.connect() as conn:
                ...
                for name in names:
                    try:
                        ...
                        tables.append({'name': name, 'rows': int(rows or 0)})
    ...
    def run_maintenance(self, action: str) -> bool:
        """
        Executa manutenção básica: 'VACUUM', 'ANALYZE' ou 'REINDEX'.
        """
        try:
            if self.engine is None:
                return False
            ...
            with self.engine.connect() as conn:
                if action_upper == 'VACUUM':
                    conn.execute(text('VACUUM'))
                elif action_upper == 'ANALYZE':
                    conn.execute(text('ANALYZE'))
                elif action_upper == 'REINDEX':
                    conn.execute(text('REINDEX'))
                else:
                    return False
            return True
        except SQLAlchemyError as e:
            self.logger.error(f"Erro na manutenção '{action}': {e}")
            return False
